...
Public Class Kiosk_Step4
    ...
    Private Sub Reserve_Step_4_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        reportinitial()
        databaseAdd()
        qrgenerate()
    End Sub
    ...
    Private Sub updateseat()
        ...
        If Movie_Details.type = "VIP" Then
            For i = 0 To KioskHome.rooms.Count - 1 Step 1
                If Kiosk_Step_1___VIP__.orderMovie.room.ID = KioskHome.rooms(i).ID Then
                    For j = 0 To KioskHome.rooms(i).showTime.Count - 1 Step 1
                        If Kiosk_Step_1___VIP__.orderMovie.room.showTime(0).time = KioskHome.rooms(i).showTime(j).time Then
                            For k = 0 To KioskHome.rooms(i).showTime(j).seat.Count - 1 Step 1
                                If Kiosk_Step_1___VIP__.orderMovie.seatSelect(k).status Then
                                    KioskHome.rooms(i).showTime(j).seat(k).status = False
                                End If
                            Next
                        End If
                    Next
        ...
        ElseIf Movie_Details.type = "Standard" Then
            For i = 0 To KioskHome.rooms.Count - 1 Step 1
                If Kiosk_Step_1__Standard_.orderMovie.room.ID = KioskHome.rooms(i).ID Then
                    For j = 0 To KioskHome.rooms(i).showTime.Count - 1 Step 1
                        If Kiosk_Step_1__Standard_.orderMovie.room.showTime(0).time = KioskHome.rooms(i).showTime(j).time Then
                            For k = 0 To KioskHome.rooms(i).showTime(j).seat.Count - 1 Step 1
                                If Kiosk_Step_1__Standard_.orderMovie.seatSelect(k).status Then
                                    KioskHome.rooms(i).showTime(j).seat(k).status = False
                                End If
                            Next
                        End If
    ...
    Private Sub databaseAdd()
        ...
        If Movie_Details.type = "VIP" Then
            For i = 0 To Kiosk_Step_1___VIP__.orderMovie.seatSelect.Count - 1 Step 1
                If Kiosk_Step_1___VIP__.orderMovie.seatSelect(i).status Then
                    ticketCount += 1
                End If
            Next

            For i = 0 To Kiosk_Step_1___VIP__.orderMovie.comboCount.Count - 1 Step 1
                comboCount += Kiosk_Step_1___VIP__.orderMovie.comboCount(i)
                DB_CinemaDataSet.Food.Rows(i).Item(4) += Kiosk_Step_1___VIP__.orderMovie.comboCount(i)
            Next

            With Kiosk_Step_1___VIP__.orderMovie
            ...
            End With
        ElseIf Movie_Details.type = "Standard" Then
            For i = 0 To Kiosk_Step_1__Standard_.orderMovie.seatSelect.Count - 1 Step 1
                If Kiosk_Step_1__Standard_.orderMovie.seatSelect(i).status Then
                    ticketCount += 1
                End If
            Next

            For i = 0 To Kiosk_Step_1__Standard_.orderMovie.comboCount.Count - 1 Step 1
                comboCount += Kiosk_Step_1__Standard_.orderMovie.comboCount(i)
                DB_CinemaDataSet.Food.Rows(i).Item(4) += Kiosk_Step_1__Standard_.orderMovie.comboCount(i)
            Next

            With Kiosk_Step_1__Standard_.orderMovie
                TransactionTableAdapter.Insert(.orderID, .memberID, .staffID, .movieSelect.id, ticketCount, comboCount, orderDate, .total)
                TransactionTableAdapter.Update(DB_CinemaDataSet)
    ...
    Private Sub reportinitial()
        Dim i As Integer
        If Movie_Details.type = "VIP" Then
            Kiosk_Step_1___VIP__.orderMovie.staffID = "Null"
            With Kiosk_Step_1___VIP__.orderMovie
        ...
        ElseIf Movie_Details.type = "Standard" Then
            Kiosk_Step_1__Standard_.orderMovie.staffID = "Null"
            With Kiosk_Step_1__Standard_.orderMovie
                lblTransaction.Text = .orderID
                lblStaff.Text = .staffID
                lblMember.Text = .memberID
            ...
            For i = 0 To Kiosk_Step_1__Standard_.orderMovie.seatSelect.Count - 1 Step 1
                If Kiosk_Step_1__Standard_.orderMovie.seatSelect(i).status Then
                    ...
                    With Kiosk_Step_1__Standard_.orderMovie
                        ...
                        e.Graphics.DrawString("Date :  " & Date.Now.ToString("dd/MM/yyyy"), font, Brushes.Black, x + 60, y)
            ...
            For i = 0 To Kiosk_Step_1___VIP__.orderMovie.seatSelect.Count - 1 Step 1
                If Kiosk_Step_1___VIP__.orderMovie.seatSelect(i).status Then
                    ...
                    With Kiosk_Step_1___VIP__.orderMovie
                        ...
                        e.Graphics.DrawString("Expiration period :  " & Date.Now.ToString("dd/MM/yyyy"), font, Brushes.DarkGoldenrod, x + 60, y)
